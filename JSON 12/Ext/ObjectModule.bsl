...
Функция ФункцияПреобразованияЗаписи(Свойство, Значение, ДополнительныеПараметры, Отказ) Экспорт
	...
	Если Найти(ИмяТипа, "EnumRef") > 0 Тогда
		...
		Если КэшСсылокАналитик <> Неопределено Тогда			
...
Процедура ВыгрузитьДанные() Экспорт
	...
	ПараметрыЗапроса.Вставить("ДатаОстатков", 	НачалоДня(ДатаНачала));
	...
	Если СтруктураМетаданныхРегистраБухгалтерии <> Неопределено Тогда
		...
		ПараметрыФала.Вставить("ПрефиксИмени", "remainds");
		ВыгрузитьОстатки(ПараметрыФала, ПараметрыЗапроса, СтруктураМетаданныхРегистраБухгалтерии, МассивМетаданных);
...
Функция ВыгрузитьОперации(ПараметрыФала, ОписаниеРегистра, ПараметрыЗапроса = Неопределено, Файлы)
	...
	Для каждого Измерение Из ОписаниеРегистра.Метаданные.Измерения Цикл
		...
		//СтруктураАналитик.Вставить(Измерение.Имя);	
	КонецЦикла;
...
Функция ВыгрузитьОстатки(ПараметрыФала, ПараметрыЗапроса, СтруктураМетаданныхРегистраБухгалтерии, МассивМетаданных) Экспорт 
	
	#Область ПолучениеСтруктурыРегистра
	
	МетаданноеРегистра = СтруктураМетаданныхРегистраБухгалтерии.МетаданныеКонфигурации;
	...
	ПоляЗапроса.Вставить("Счет", "AccountRemaining.AccountId");
	
	Для каждого Ресурс Из МетаданноеРегистра.Ресурсы Цикл
		
		Если КритерийРесурсаКоличество = Ресурс.Имя Тогда
			ДобавитьПоляОстатка(ПоляЗапроса, Ресурс, "AccountRemaining.", "Quantity");
		ИначеЕсли КритерийРесурсаСуммы = Ресурс.Имя Тогда
			ДобавитьПоляОстатка(ПоляЗапроса, Ресурс, "AccountRemaining.", "Remaining");
		ИначеЕсли КритерийРесурсаВалютнаяСумма = Ресурс.Имя Тогда
			ДобавитьПоляОстатка(ПоляЗапроса, Ресурс, "AccountRemaining.", "CurrencyRemaining");
		КонецЕсли;				
	...
	Для каждого Реквизит Из МетаданноеРегистра.СтандартныеРеквизиты  Цикл			
		...
		Если Найти(Реквизит.Имя, "Субконто") > 0 И НЕ Найти(Реквизит.Имя, "ВидСубконто") > 0 Тогда				
			...
			ПоляЗапроса.Вставить(Реквизит.Имя, "AccountRemainingAnalytics.AnalyticsId");
			СтруктураАналитик.Вставить(Реквизит.Имя);
			
			Структура = Новый Структура;
			Структура.Вставить(Реквизит.Имя, "AccountRemainingAnalytics.AnalyticsId");
			Структура.Вставить("Тип" + Реквизит.Имя, "AccountRemainingAnalytics.AnalyticsType");
			
			МетаданныеОпераций.Субконто.Добавить(Структура);
			МассивПолейАналитик.Добавить(Реквизит.Имя);
	...
	Для каждого Измерение Из МетаданноеРегистра.Измерения Цикл
		...
		Если КритерийИзмеренияОрганизаций = Измерение.Имя Тогда
			
			ПоляЗапроса.Вставить(Измерение.Имя, "AccountRemaining.BranchId");
			СтруктураИзмерений.Вставить(Измерение.Имя);
			
		ИначеЕсли КритерийИзмеренияВалюта = Измерение.Имя Тогда
			
			ПоляЗапроса.Вставить(Измерение.Имя, "AccountRemainingAnalytics.AnalyticsId");
			СтруктураИзмерений.Вставить(Измерение.Имя);
	...
	#Область Получение_Данных
		...
		Текст = Текст + Символы.ПС
			+"ИЗ
			|РегистрБухгалтерии.#ИмяРегистра.Остатки(&ДатаОстатков, , , ) КАК Операция";	

		Текст = СтрЗаменить(Текст, "#ИмяРегистра", МетаданноеРегистра.Имя);
			 	                                       
		Запрос = Новый Запрос(Текст);	
		Запрос.УстановитьПараметр("ДатаОстатков", ПараметрыЗапроса.ДатаОстатков);
	...
	СтруктураЗаписи = Новый Структура(Новый ФиксированнаяСтруктура(ПоляЗапроса));
	Для каждого элемент Из МассивПолейАналитик Цикл
		СтруктураЗаписи.Вставить("Тип" + элемент);	
	КонецЦикла;
	...
	Пока ТребуетсяЗаписьОпераций Цикл
		...
		Пока ВыборкаОпераций.Следующий() Цикл
			...
			Для каждого элемент Из МассивПолейАналитик Цикл
				...
				ТипXML = XMLТипЗнч(СтруктураЗаписи[элемент]);
				ИмяТипа = NULL;
				Если ТипXML <> Неопределено и ЗначениеЗаполнено(СтруктураЗаписи[элемент]) Тогда
					ИмяТипа = ТипXML.ИмяТипа;
				КонецЕсли;
				
				СтруктураЗаписи.Вставить("Тип" + элемент, ИмяТипа);
	...
	#Область ЗаписьМетаданных
		...
		СтруктураОписания.Вставить("ВидМетаданных", "Остатки");
		СтруктураОписания.Вставить("Файлы", 		файлы);
		СтруктураОписания.Вставить("Синоним", 		МетаданноеРегистра.Синоним);
		СтруктураОписания.Вставить("ДатаОстатков", 	ПараметрыЗапроса.ДатаОстатков);
...
Процедура ДобавитьПоляОстатка(Структура, Поле, ПрефиксИмени = "", ПостфиксИмени = "")
		
	Структура.Вставить(Поле.Имя + "ОстатокДт", ПрефиксИмени + "Debit" 	+ ПостфиксИмени);
	Структура.Вставить(Поле.Имя + "ОстатокКт", ПрефиксИмени + "Credit" + ПостфиксИмени);
